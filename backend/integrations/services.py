...
def apply_artwork_fallbacks(
    *,
    ...
    libraries = {
        str(item.id): item
        for item in Library.objects.filter(
            owner=user,
            id__in=by_library.keys(),
        )
    }
    ...
    for (
        ...
        links = active_links_for_library(
            library=library,
            capability="artwork",
        )
